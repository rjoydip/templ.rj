...
describe('@templ/utils > Obj', () => {
  describe('includeKeys', () => {
    test('function predicate returns a boolean', () => {
      expect(Object.keys(includeKeys({ foo: true, bar: false }, () => true)).length).toBe(2)
      expect(Object.keys(includeKeys({ foo: true, bar: false }, () => false)).length).toBe(0)
    })

    test('function predicate passes the key as argument', () => {
      expect(includeKeys({ foo: true }, key => key === 'foo').foo).toBeTruthy()
    })

    test('function predicate passes the value as argument', () => {
      expect(includeKeys({ foo: 'test' }, (_, value) => value === 'test').foo).toBe('test')
    })

    test('function predicate passes the object as argument', () => {
      expect(includeKeys({ foo: true }, (_, __, object) => object.foo).foo).toBeTruthy()
    })
    ...
    test('descriptors are kept as is', () => {
      ...
      expect(Object.getOwnPropertyDescriptor(includeKeys(input, () => true), 'test')).toStrictEqual(descriptor)
  ...
  describe('excludeKeys', () => {
    test('excludeKeys: function predicate returns a boolean', () => {
      expect(Object.keys(excludeKeys({ foo: true, bar: false }, () => true)).length).toBe(0)
      expect(Object.keys(excludeKeys({ foo: true, bar: false }, () => false)).length).toBe(2)
    })

    test('excludeKeys: function predicate passes the key as argument', () => {
      expect(excludeKeys({ foo: true }, key => key !== 'foo').foo).toBeTruthy()
    })

    test('excludeKeys: function predicate passes the value as argument', () => {
      expect(excludeKeys({ foo: 'test' }, (key, value) => value !== 'test').foo, 'test').toBeTruthy()
    })

    test('excludeKeys: function predicate passes the object as argument', () => {
      expect(excludeKeys({ foo: true }, (key, value, object) => !object.foo).foo).toBeTruthy()
    })
    ...
    test('excludeKeys: descriptors are kept as is', () => {
      const descriptor = { get() { }, set() { }, enumerable: true, configurable: false }
      const input = Object.defineProperty({}, 'test', descriptor)
      expect(Object.getOwnPropertyDescriptor(excludeKeys(input, () => false), 'test')).toStrictEqual(descriptor)
      // TODO: Having problem in predicate
