...
> extends BaseOptions<TShared> {
  runtimeFlag: Record<string, string | boolean | number | undefined>
}
...
export function createFlag<
  ...
  TShared extends SharedType,
>(
  opts: FlagOptions<TPrefix, TServer, TClient, TShared>,
  ...
  const parsed = isServer
    ? z.object(opts.server && typeof opts.server === 'object'
      ? opts.server
      : {}).merge(shared).merge(client).safeParse(opts.runtimeFlag)
    : client.merge(shared).safeParse(opts.runtimeFlag)
  ...
  const onValidationError
    ...
    ?? ((error: ZodError) => {
      if (!isTest) {
        console.error(
          '❌ Invalid feature flag variables:',
          error.flatten().fieldErrors,
        )
      }
      throw new Error('Invalid feature flag variables')
    })
  ...
  const onInvalidAccess
    ...
    ?? ((_variable: string) => {
      throw new Error(
        '❌ Attempted to access a server-side feature flag variable on the client',
      )
    })
